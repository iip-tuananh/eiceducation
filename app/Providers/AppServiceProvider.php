...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        view()->composer('*', function ($view) 
        {
            ...
            $servicehome = Solution::where('status',1)->orderBy('id','ASC')->get();
            // $duhoccacnuoc = Services::where('status',1)->orderBy('id','ASC')->get();
            $duhoccacnuoc = ServiceCate::where('status',1)->orderBy('id','DESC')->get();
